...
from Remote_User.models import ClientRegister_Model,heartrate_model,hrpredict_model
...
def Add_DataSet_Details(request):
    ...
    for r in range(1, active_sheet.max_row+1):
        heartrate_model.objects.create(
        names= active_sheet.cell(r, 1).value,
        VLF= active_sheet.cell(r, 2).value,
        VLF_PCT= active_sheet.cell(r, 3).value,
        LF= active_sheet.cell(r, 4).value,
        LF_PCT= active_sheet.cell(r, 5).value,
        LF_NU= active_sheet.cell(r, 6).value,
        HF= active_sheet.cell(r, 7).value,
        HF_PCT= active_sheet.cell(r, 8).value,
        HF_NU= active_sheet.cell(r, 9).value,
        TP= active_sheet.cell(r, 10).value,
        LF_HF= active_sheet.cell(r, 11).value,
        HF_LF= active_sheet.cell(r, 12).value
        )
...
def Search_HeartRate_DataSets(request):
    if request.method == "POST":
        ...
        if request.method == "POST":
            ...
            pred11 = str(pred1.replace(".", ""))

            prediction = int(pred11)

            if prediction == 0:
                val = 'High Level Stress'
            elif prediction == 1:
                val = 'Midium Level Stress'
            elif prediction == 2:
                val = 'Low Level Stress'
        ...
        return render(request, 'RUser/Search_HeartRate_DataSets.html',{'objs': val})
    return render(request, 'RUser/Search_HeartRate_DataSets.html')


def ratings(request,pk):
    vott1, vott, neg = 0, 0, 0
    objs = hrpredict_model.objects.get(id=pk)
    unid = objs.id
    vot_count = hrpredict_model.objects.all().filter(id=unid)
    for t in vot_count:
        vott = t.ratings
        vott1 = vott + 1
        obj = get_object_or_404(hrpredict_model, id=unid)
        obj.ratings = vott1
